...
var validCases = []rule_tester.ValidTestCase{
	{
		FileName: "packages/coffee/application/src/use-cases/checkoutSession.ts",
		Code: `
const expiresAt = DateTime.add(now, { minutes: 15 });
		`,
	},
	{
		FileName: "packages/coffee/application/src/use-cases/checkoutSession.test.ts",
		Code: `
const now = new Date();
		`,
	},
...
const expiresAt = new Date(Date.now() + 900000);
	...
	{
		FileName: "packages/coffee/domain/src/checkout-session.ts",
		Code: `
