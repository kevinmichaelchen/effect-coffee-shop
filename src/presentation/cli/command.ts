...
const createOrder = Command.make("create", {
  customerName,
  drink: Flag.choice("drink", drinkIds).pipe(
    Flag.withDescription("Drink identifier")
  ),
  size: Flag.choice("size", drinkSizes).pipe(
  ...
  Command.withHandler(Effect.fn("CoffeeCli.createOrder")(function*({
    customerName,
    drink,
    size,
    milk,
...
const getOrderCommand = Command.make("get", {
  orderId: Flag.string("order-id").pipe(
    Flag.withAlias("i")
  )
}).pipe(
  Command.withDescription("Fetch an order by id"),
  Command.withHandler(Effect.fn("CoffeeCli.getOrder")(function*({ orderId }) {
...
const listOrdersCommand = Command.make("list", {
  ...
  Command.withHandler(Effect.fn("CoffeeCli.listOrders")(function*({ status }) {
...
const cancelOrderCommand = Command.make("cancel", {
  ...
  Command.withHandler(Effect.fn("CoffeeCli.cancelOrder")(function*({ orderId }) {
...
const listMenuCommand = Command.make("list").pipe(
  ...
  Command.withHandler(Effect.fn("CoffeeCli.listMenu")(function*() {
...
const startCommand = Command.make("start", {
  ...
  Command.withHandler(Effect.fn("CoffeeCli.startBrewing")(function*({ orderId }) {
...
const readyCommand = Command.make("ready", {
  ...
  Command.withHandler(Effect.fn("CoffeeCli.markReady")(function*({ orderId }) {
...
const pickupCommand = Command.make("pickup", {
  ...
  Command.withHandler(Effect.fn("CoffeeCli.pickUpOrder")(function*({ orderId }) {
...
export const runCoffeeCli = Command.run(coffeeCli, {
  ...
  Effect.provide(InMemoryCoffeeAppLive),
