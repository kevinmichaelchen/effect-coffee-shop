...
const PlaceOrderErrorSchema = Schema.Union([
  DrinkNotFoundError,
  InvalidOrderInputError
]).annotate({ identifier: "PlaceOrderError" })
...
class MenuApi extends HttpApiGroup.make("menu").add(
  HttpApiEndpoint.get("list", "/", {
    success: MenuSchema
  })
).prefix("/menu") {}

class OrdersApi extends HttpApiGroup.make("orders").add(
...
const HealthApiLive = HttpApiBuilder.group(
  CoffeeHttpApi,
  "health",
  (handlers) => handlers.handle("check", () => Effect.succeed(HEALTH_STATUS))
)

const MenuApiLive = HttpApiBuilder.group(
  CoffeeHttpApi,
  "menu",
  (handlers) => handlers.handle("list", () => listMenu())
)

const OrdersApiLive = HttpApiBuilder.group(
  CoffeeHttpApi,
  "orders",
  (handlers) =>
    handlers
